...
// Remove direct useSearchParams at top level
...
const Contact = () => {
  ...
  // Only import and use useSearchParams on the client
  useEffect(() => {
    if (!mounted) return;
        ...
        <div className="w-full lg:w-[95%] bg-white rounded-2xl shadow-lg p-8 grid md:grid-cols-2 gap-10">
          ...
          <form
            ...
            onSubmit={(e) => {
              ...
              if (!firstName || !lastName) return alert("Please fill in both First Name and Last Name.");
          ...
          >
            ...
            <div>
              ...
              <textarea rows="4" placeholder="Write Your Message" className="w-full border-b border-gray-300 focus:outline-none focus:border-black py-2 resize-none" />
            </div>
            <button type="submit" className="bg-[#FF3600] text-white px-8 py-3 rounded-full hover:bg-[#FF3600]/60 active:bg-[#FF3600]/60 transition">Send Message</button>
          </form>
        </div>
      </div>
...
const InputField = ({ label, placeholder, type = "text" }) => (
  <div>
    ...
    <input type={type} placeholder={placeholder} className="w-full border-b border-gray-300 focus:outline-none focus:border-black py-2" />
  </div>
);

const SocialIcon = ({ link, children }) => (
  <a href={link} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-full border border-gray-600 hover:border-orange-500 hover:text-orange-500 transition">
    {children}
  </a>
);
